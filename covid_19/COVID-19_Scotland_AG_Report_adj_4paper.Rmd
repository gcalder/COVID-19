...
last_day <- dmy("10/06/2020") ### CHANGE LAST DATE
...
# Import NRS data
myurl_deaths <- "https://www.nrscotland.gov.uk/files//statistics/covid19/covid-deaths-data-week-28.xlsx"
...
nrs_deaths_covid <- nrs_deaths_covid_raw %>% filter(`Week beginning` == "Deaths involving COVID-194") %>%
  ...
  filter(week_beginning <= (last_day-days(5))) 
...
#HB ICU
hb_icu <- read_excel(tmp, sheet = "Table 2a - ICU patients", skip = 2) %>%
...
fig2 <- ggplot(wk_gr_newdeaths_adj_mainplot %>% filter(!is.na(ratio) & 
  ...
  scale_y_continuous(trans = "log10",
                         expand = expansion(mult = 0.1),
                     breaks = c(0.1, 0.2, 0.5, 1,3,10,30, 60),
                     limits = c(NA, 60),
...
fig3 <- ggdraw() +
  draw_plot(fig3) +
  draw_plot(fig3_inset, x = 0.51, y = .55, width = .44, height = .24)
...
fig4 <- ggdraw() +
  draw_plot(fig4) +
  draw_plot(fig4_inset, x = 0.51, y = .57, width = .44, height = .24)
...
```{r deathscaseshospicupaper, fig.width=5.5, fig.height= 9}
pdf("epidemic_curves_4metrics_4paper.pdf", width = 8, height = 10)
fig1 <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_blue, alpha = 0.4) +
...
fig1_inset <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & 
  ...
  geom_point(colour = nice_blue, size = 1) +
  scale_x_date(date_breaks = "10 days", labels = date_format("%b %d")) +
  scale_y_continuous(trans = "log10", limits = c(NA, 1.1)) + # CHANGE
  labs( x = "", y = "") +
  theme_bw(base_size = 8) +
  theme(plot.margin = margin(0, 0, -1, 0, "cm"))
...
fig2 <- ggplot(wk_gr_newdeaths_adj_mainplot %>% filter(!is.na(ratio) & 
  ...
  scale_y_continuous(trans = "log10",
                         expand = expansion(mult = 0.1),
                     breaks = c(0.1, 0.2, 0.5, 1,3,10,30),
                     limits = c(NA, 30),
                     #labels = c(0.5, 1,3,10,30)
                     ) +
  scale_x_date(date_breaks = "2 weeks", labels = date_format("%b %d"),
...
fig2 <- ggdraw() +
  draw_plot(fig2) +
  draw_plot(fig2_inset, x = 0.51, y = .57, width = .44, height = .24)
...
```{r ratiosnrshpslocpaper, fig.width=8}
pdf("epidemic_curves_deathsbylocratios_4paper.pdf", width = 10, height = 6)
fig1 <- 
  bind_rows(wk_gr_nps_nrs, wk_gr_newdeaths_latest) %>%
  filter(Location != "Home / Non-institution") %>%
  mutate(Location = factor(Location, levels = c("HPS", "NRS","Care Home", "Hospital"))) %>%
...
ggdraw() +
  draw_plot(fig1) +
  draw_plot(fig1_inset, x = 0.38, y = .48, width = .33, height = .35)
...
```{r ratiosnrshpsdailypaper, fig.width=8}

pdf("epidemic_curves_deathsbysourceratios_4paper.pdf", width = 10, height = 6)
# wk_gr_newdeaths_nrs_hps %>% 
#   filter(!is.na(ratio) & date >= dmy("26/04/2020")) %>%
# mutate(Ratio = formatC(round(ratio_m,2), format='f', digits=2),
#          lci = formatC(round(lci,2), format='f', digits=2),
