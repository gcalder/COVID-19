...
wk2_gr_hosp_icu <- data_hosp %>%
       mutate(ICU_prev = lag(ICU_total , 14), 
         weekdays(date)) %>%
       filter(date >= ymd("2020-03-31")) %>%
       pivot_longer(c(ICU_total, ICU_prev), names_to = "week", values_to = "number") %>%
      group_by(date) %>%
      nest() %>%
      mutate(mod = map(data, ~ glm(number ~ week,
...
wk2_gr_hosp_hosp <- data_hosp %>%
       mutate(Hosp_prev = lag(Hospital_total , 14), 
         weekdays(date)) %>%
       filter(date >= ymd("2020-03-31")) %>%
       pivot_longer(c(Hospital_total, Hosp_prev), names_to = "week", values_to = "number") %>%
      group_by(date) %>%
...
path <- paste0("Daily_Reports/COVID-19_Scotland_data_all_", {Sys.Date()-2}, ".xlsx") 
# Scottish cases data
...
# 14 day ratios
wk2_gr_newcases <- scot_data %>% select(new_cases, date) %>%
      mutate(outcome = new_cases) %>%
       weekly_ratios(smooth_by = 14) %>%
       filter(!is.na(ratio) & !is.infinite(ratio)) %>%
       significance_wk() %>%
       mutate(adjustment = "no adjustment")

wk2_gr_newdeaths <- scot_deaths %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths) %>%
       weekly_ratios(smooth_by = 14) %>%
       filter(!is.na(ratio) & !is.infinite(ratio)) %>%
       significance_wk()

wk2_gr_newdeaths_nrs <- nrs_deaths_covid_raw_daily %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths) %>%
       weekly_ratios(smooth_by = 14) %>%
       filter(!is.na(ratio) & !is.infinite(ratio)) %>%
       significance_wk()
...
gr_newdeaths_nrs <- growth_rates(dat = nrs_deaths_covid_raw_daily %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths)) %>%
  significance() 

# Growth rates - 7 days CUMULATIVE CASES 
gr_cumcases <- growth_rates(dat = scot_data %>% rename("outcome" = confirmed_cases)) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr_cumdeaths <- growth_rates(dat = scot_deaths %>% rename("outcome" = deaths)) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr_cumdeaths_nrs <- growth_rates(dat = nrs_deaths_covid_raw_daily %>% 
      mutate(outcome = `Cumulative Count`)) %>%
  significance() 

# Growth rates - 14 days
gr14_newcases <- growth_rates(dat = scot_data %>% rename("outcome" = new_cases), days_n = 13) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr14_newdeaths <- growth_rates(dat = scot_deaths %>% rename("outcome" = new_deaths), days_n = 13) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr14_newdeaths_nrs <- growth_rates(dat = nrs_deaths_covid_raw_daily %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths), days_n = 13) %>%
  significance() 

# Growth rates - 21 days
gr21_newcases <- growth_rates(dat = scot_data %>% rename("outcome" = new_cases), days_n = 20) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr21_newdeaths <- growth_rates(dat = scot_deaths %>% rename("outcome" = new_deaths), days_n = 20) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr21_newdeaths_nrs <- growth_rates(dat = nrs_deaths_covid_raw_daily %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths), days_n = 20) %>%
  significance() 

# Growth rates - 28 days
gr28_newcases <- growth_rates(dat = scot_data %>% rename("outcome" = new_cases), days_n = 27) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr28_newdeaths <- growth_rates(dat = scot_deaths %>% rename("outcome" = new_deaths), days_n = 27) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance() 

gr28_newdeaths_nrs <- growth_rates(dat = nrs_deaths_covid_raw_daily %>% select(new_deaths, date) %>%
      mutate(outcome = new_deaths), days_n = 27) %>%
  significance()
```


Numbers of new cases and new deaths, as reported by Health Protection Scotland (HPS), have been monitored on a daily basis (see Figures \@ref(fig:traj) and \@ref(fig:dailydeathscases)). To assess the rate of spread of COVID-19 in Scotland, we have considered the ratio of the sum of these metrics in the past week compared to their respective sums in the previous week. A similar ratio has been used to measure changes in hospital bed occupancy by comparing daily numbers of bed and ICU occupancy to those one week ago (see Figure \@ref(fig:deathscaseshospicu)). However, these statistics are potentially subject to biases and we list several caveats which should be considered when interpreting their values.
...
The number of COVID-19 patients in hospital and Intensive Care Units (ICUs) are reported on a daily basis. The terms 'hospital occupancy' and 'ICU occupancy' refer to the number of COVID-19 patients who are registered as being under that department's care at midnight (see Figure \@ref(fig:dailyoccupancy)).

\subsection*{Weekly comparisons}
We have used ratios of both cases and deaths in the past week compared to the previous week (i.e. the total over the 7 days as a ratio of the total over the previous 7 days) to monitor the weekly rate of spread of COVID-19 (see Figures \@ref(fig:deathscaseshospicu), \@ref(fig:ratiosnrshpsloc), \@ref(fig:ratiosnrshpsdaily) and Table \@ref(tab:tabdeathscaseshospicu)). Weekly totals are considered as they are less prone to week day variations than the daily values (see Caveats below).
...
-	Variations in reporting of deaths and test results across week days and due to public holidays (See Figure \@ref(fig:dayoftheweeklast2weeks)).
...
An increase in testing is likely to cause less severely infected cases to be included and this may make comparisons of case numbers over time unreliable. Nevertheless, a decrease in cases would still provide strong evidence of a reducing rate of infection.
...
HPS death numbers include individuals who have positive tests but are not in hospital. Increased testing in the community (particularly in care homes) will cause more deaths to be counted and make HPS death numbers difficult to compare over time. The NRS death numbers are expected to allow a more consistent comparison, and also allow care home and other non-hospital deaths to be considered separately. However, these data are only issued on a weekly basis and they will be less up-to-date than the data from HPS. The latest weekly ratio for NRS deaths (available to `r latest_nrs_hps_date`), `r filter(latest_nrs_hps, Location == "NRS")$ratio_m` (95% CI `r filter(latest_nrs_hps, Location == "NRS")$lci`-`r filter(latest_nrs_hps, Location == "NRS")$uci`), is now noticeably lower than that for HPS deaths `r filter(latest_nrs_hps, Location == "HPS")$ratio_m` (95% CI `r filter(latest_nrs_hps, Location == "HPS")$lci`-`r filter(latest_nrs_hps, Location == "HPS")$uci`) and might be expected to more accurately reflect the true underlying rate of spread of COVID-19 (see Figures \@ref(fig:ratiosnrshpsloc) and \@ref(fig:ratiosnrshpsdaily)).

Given the later availability of NRS data and the variable factors affecting both the number of cases and HPS deaths, some consideration might be given to hospital occupancy data (see Figure \@ref(fig:deathscaseshospicu)). However, these data will not reflect deaths in care homes, and will be affected by any change in criteria for admission to hospital or to ICU. The recent increase in the numbers of patients taken to hospital by ambulance coupled with a lower number of cases, may indicate that admission criteria have been relaxed recently and a higher proportion of COVID-19 cases are being admitted in hospital.
...
The ratios by place of death (see Figure \@ref(fig:ratiosnrshpsloc)) will also be subject to any variation in hospital admission policy. For example, if the proportion of care home residents being admitted to hospital has increased over a two week period, we might expect the ratio for deaths in care homes to be an under-estimate, and the ratio for hospitals an over-estimate.
...
arrow_date <- max(cases_deaths$date)-20
...
```{r dailydeathscases, fig.width=8, fig.height=3.5, fig.cap = "Daily cases and deaths and 7 day rolling sum"}
...
ggplot() + 
  ...
  geom_line(aes(x = date, y = rolling_sum/ymax), colour = "red") +
  scale_y_continuous(name = 'New Deaths', sec.axis = sec_axis(~.*ymax, 
                                                                   name = "7 day rolling sum")) +
  labs(x = "Date") +
  theme_bw()
...
```{r dailyoccupancy, fig.width=6, fig.height=3, fig.cap="Hospital and ICU occupancy at midnight. Daily totals include both confirmed and suspected COVID-19 cases"}
fig1 <- data_hosp_icu %>%
  group_by(date) %>%
  summarise(Number = sum(Number, na.rm = T)) %>%
  ggplot() + 
  geom_bar(aes(x = date, y = Number), stat = "identity", colour = "white", size = 0.2) +
  theme_bw() +
  labs(x = "Date", y = "ICU Occupancy") +
...
fig2 <- data_hosp_hosp %>%
  ...
  theme_bw() +
  labs(x = "Date", y = "Hospital Occupancy") +
  theme(legend.position = c(0.17, 0.8), 
        legend.title = element_blank()) +
  theme(plot.margin = unit(c(0,0,0,25), "pt"))
...
fig1 <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("23-03-2020"))) +
  ...
  scale_x_date(date_breaks = "1 week", labels = date_format("%b %d"), expansion(mult = 0.02)) +
  scale_y_continuous(trans = "log10",
                         labels = scales::comma_format(accuracy = 1),
                         #breaks = breaks,
...
```{r deathscaseshospicu2weeks, fig.width=5.5, fig.height= 9, fig.cap="14-day ratios for new cases and new deaths are ratios of weekly sums, plotted on a log10 scale. Day-of-the-week ratios for hospital occupancy and ICU occupancy are ratios of daily counts, measured 14 days apart and plotted on a log10 scale. Figure insets highlight more recent ratios and trends."}
fig1 <- ggplot(wk2_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_blue, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  geom_line(colour = nice_blue) +
  geom_point(colour = nice_blue, size = 1) +
  scale_x_date(date_breaks = "1 week", labels = date_format("%b %d"),expand = expansion(mult = 0.02)) +
  scale_y_continuous(trans = "log10",
                         labels = scales::comma_format(accuracy = 1),
                         #breaks = breaks,
                         expand = expansion(mult = 0.1)) +
...
fig1_inset <- ggplot(wk2_gr_newcases %>% filter(!is.na(ratio) & 
                    !is.infinite(ratio) & date >= max(wk_gr_newcases$date)-8)) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_blue, alpha = 0.4) +
...
fig2 <- ggplot(wk2_gr_newdeaths %>% filter(!is.na(ratio) & 
              !is.infinite(ratio) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_red, alpha = 0.4) +
...
fig2_inset <- ggplot(wk2_gr_newdeaths %>% filter(!is.na(ratio) & 
                    !is.infinite(ratio) & date >= max(wk_gr_newdeaths$date)-8)) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_red, alpha = 0.4) +
...
fig3 <- ggplot(wk2_gr_hosp_icu %>% filter(!is.na(ratio_m) & 
              !is.infinite(ratio_m) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_orange, alpha = 0.4) +
...
fig3_inset <- ggplot(wk2_gr_hosp_icu %>% filter(!is.na(ratio_m) & 
                    !is.infinite(ratio_m) & date >= max(wk_gr_hosp_icu$date)-8)) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_orange, alpha = 0.4) +
...
fig4 <- ggplot(wk2_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
              !is.infinite(ratio_m) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_green, alpha = 0.4) +
...
fig4_inset <- ggplot(wk2_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
                    !is.infinite(ratio_m) & date >= max(wk_gr_hosp_hosp$date)-8)) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_green, alpha = 0.4) +
...
fig1 <- ggplot(gr14_newcases) +
  ...
  scale_y_continuous(expand = c(0.1, 0), #breaks = c(-0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8), 
                     ...
                     limits = c(-20, NA)) +
  labs(
    x = "End date of 14 day period",
    y = "Daily % growth",
    title = "Daily growth estimated over 14 day window",
    subtitle = "New cases"
  ) +
  guides(color = guide_legend(nrow = 4)) +
  theme_bw(base_size = 12) +
...
fig1 <- ggplot(gr21_newcases) +
  ...
  scale_y_continuous(expand = c(0.1, 0), #breaks = c(-0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8), 
                     ...
                     limits = c(-20, NA)) +
  labs(
    x = "End date of 21 day period",
    y = "Daily % growth",
    title = "Daily growth estimated over 21 day window",
    subtitle = "New cases"
  ) +
  guides(color = guide_legend(nrow = 4)) +
  theme_bw(base_size = 12) +
...
fig1 <- ggplot(gr28_newcases) +
  ...
  scale_y_continuous(expand = c(0.1, 0), #breaks = c(-0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8), 
                     ...
                     limits = c(-20, NA)) +
  labs(
    x = "End date of 28 day period",
    y = "Daily % growth",
    title = "Daily growth estimated over 28 day window",
    subtitle = "New cases"
  ) +
  guides(color = guide_legend(nrow = 4)) +
  theme_bw(base_size = 12) +
...
wk_gr_hosp_hosp %>% filter(date == max(wk_gr_hosp_hosp$date)) %>% mutate(Measure = "Hospital Occupancy (Source: NHS Boards)"),
wk_gr_hosp_icu %>% filter(date == max(wk_gr_hosp_icu$date)) %>% mutate(Measure = "ICU Occupancy (Source: NHS Boards)")) %>%
...
knitr::kable(
  ...
  caption = str_glue("Most recent estimates of ratios and their 95\\% confidence intervals (CI). Weekly ratios for new cases and new deaths are ratios of weekly sums. Day-of-the-week ratios for hospital occupancy and ICU occupancy are ratios of daily counts, measured 7 days apart. Date: ", format(max(wk_gr_newcases$date), '%d/%m/%Y'),"."))
```
...
```{r dayoftheweeklast2weeks, fig.width=6, fig.height=3.5, fig.cap="Number of cases and deaths by week day, shown for the last two weeks."}
