...
ggsave("weekly_ratios_cases_age_group_ci.png", width = 9, height = 6, units = "in", dpi = 200)
ggsave("weekly_ratios_cases_age_group_noci.png", width = 9, height = 6, units = "in", dpi = 200)
...
ca_pop <- read_csv("council-area-profiles-dataset-with-copyright/council-area-profiles-dataset-with-copyright_population-estimates.csv") %>%
  filter(Year == 2019)
...
<!--Graphs of frequencies and ratios are provided separately for each health board in Figures \@ref(fig:cases) to \@ref(fig:deaths).
-->
...
```{r, fig.width=11, fig.height=7, dailydeathscases, fig.cap = "Daily cases, deaths and positivity rate and 7 day rolling averages. Positivity rate refers to the percentage of people testing positive out of the total number of people tested each day."}
...
```{r deathscaseshospicuratioportrait}
  fig1 <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("3-09-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
geom_errorbar(width = 0, colour = nice_blue, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  ...
  geom_point(colour = nice_blue, size = 0.5) +
  scale_x_date(date_breaks = "2 weeks", labels = date_format("%b %d"),
               limits = c(dmy("2-09-2020"), NA), expand = c(0.015, 0)) +
  scale_y_continuous(trans = "log10",
                         breaks = c(0.5, 1,2,3),
                         limits = c(NA, 3.2),
                         #labels = c(0.5, 1,3,6),
...
fig2 <- ggplot(wk_gr_newdeaths_adj_mainplot %>% filter(!is.na(ratio) & 
              !is.infinite(ratio) & date >= dmy("03-09-2020"))) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, alpha = 0.4, aes(colour = ci)) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
...
fig3 <- ggplot(wk_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
              !is.infinite(ratio_m) & date >= dmy("3-09-2020"))) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_green, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.1) +
...
png("weekly_ratios_4_metrics.png", height = 11.69, width = 7, units = "in", res = 400)
...
```{r deathscaseshospicuratiotimeline}
  fig1 <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("3-09-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_vline(xintercept=(dmy("29/8/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Schools re-open (Aug 11)",y = 3.2, x = dmy("29/8/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_segment(aes(x = dmy("29/8/2020"), y = 9.5, xend = dmy("27/8/2020"), yend = 9.5),
                  arrow = arrow(length = unit(0.1, "cm"))) +
  geom_vline(xintercept=(dmy("1/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Local restrictions",y = 4, x = dmy("1/9/2020"), angle=90, size = 3, vjust= -0.5) +  
  geom_vline(xintercept=(dmy("13/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Universities re-open",y = 4, x = dmy("13/9/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_vline(xintercept=(dmy("14/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Rule of 6",y = 4, x = dmy("14/9/2020"), angle=90, size = 3, vjust= 1.5) +  
  geom_vline(xintercept=(dmy("25/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Pubs to close early",y = 4, x = dmy("25/9/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_vline(xintercept=(dmy("9/10/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Hospitality & \nCentral Belt restrictions",y = 4, x = dmy("9/10/2020"), angle=90, size = 3, vjust= -0.2, lineheight = 1) +geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Tier system",y = 4, x = dmy("2/11/2020"), angle=90, size = 3, vjust= -0.5) +  
geom_errorbar(width = 0, colour = nice_blue, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  geom_line(colour = nice_blue) +
  geom_point(colour = nice_blue, size = 0.5) +
  scale_x_date(date_breaks = "2 weeks", labels = date_format("%b %d"),
               limits = c(dmy("27-08-2020"), NA), expand = c(0.015, 0)) +
  scale_y_continuous(trans = "log10",
                         breaks = c(0.5, 1,2,3, 6),
                         limits = c(NA, 10),
                         expand = expansion(mult = 0.1)) +
  labs( x = "End Date of Current Week",
        y = "Weekly Ratio",
...
df_school_hols <- data.frame(start = c(dmy("14/09/2020"), dmy("05/10/2020")), end = c(dmy("28/09/2020"), dmy("26/10/2020")), ymin=-Inf, ymax = Inf)



fig1_shade <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & date >= dmy("3-09-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_rect(xmin=dmy("14/09/2020"), xmax=dmy("28/09/2020"), ymin=-Inf, ymax=Inf, fill = "thistle2",alpha =0.02 ) +
  geom_rect(xmin=dmy("5/10/2020"), xmax=dmy("26/10/2020"), ymin=-Inf, ymax=Inf, fill = "thistle2",alpha = 0.02) +
  geom_vline(xintercept=(dmy("29/8/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Schools re-open (Aug 11)",y = 3.2, x = dmy("29/8/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_segment(aes(x = dmy("29/8/2020"), y = 9.5, xend = dmy("27/8/2020"), yend = 9.5),
                  arrow = arrow(length = unit(0.1, "cm"))) +
...
ggsave("weekly_ratios_cases_timeline_school_closures_shade.png", fig1_shade, width = 9, height = 2.8, units = "in", dpi = 400)
...
fig2 <- ggplot(wk_gr_newdeaths_adj_mainplot %>% filter(!is.na(ratio) & 
  ...
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
    geom_vline(xintercept=(dmy("29/8/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Schools re-open (Aug 11)",y = 9, x = dmy("29/8/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_segment(aes(x = dmy("29/8/2020"), y = 40, xend = dmy("27/8/2020"), yend = 40),
                  arrow = arrow(length = unit(0.1, "cm"))) +
  geom_vline(xintercept=(dmy("1/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Local restrictions",y = 13, x = dmy("1/9/2020"), angle=90, size = 3, vjust= -0.5) +  
  geom_vline(xintercept=(dmy("13/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Universities re-open",y = 13, x = dmy("13/9/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_vline(xintercept=(dmy("14/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Rule of 6",y = 13, x = dmy("14/9/2020"), angle=90, size = 3, vjust= 1.5) +  
  geom_vline(xintercept=(dmy("25/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Pubs to close early",y = 13, x = dmy("25/9/2020"), angle=90, size = 3, vjust= -0.5) +   geom_vline(xintercept=(dmy("9/10/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Hospitality & \nCentral Belt restrictions",y = 13, x = dmy("9/10/2020"), angle=90, size = 3, vjust= -0.2, lineheight = 1) +geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Tier system",y = 13, x = dmy("2/11/2020"), angle=90, size = 3, vjust= -0.5) + 
  geom_errorbar(width = 0, alpha = 0.4, aes(colour = ci)) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  geom_line(aes(colour = line, group = 1)) +
  geom_point(size = 0.5, aes(colour = ci)) +
...
png("weekly_ratios_cases_deaths_timeline.png", width = 9, height = 6, units = "in", res = 400)
...
fig3 <- ggplot(wk_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
  ...
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_vline(xintercept=(dmy("29/8/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Schools re-open (Aug 11)",y = 5.5, x = dmy("29/8/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_segment(aes(x = dmy("29/8/2020"), y = 20, xend = dmy("27/8/2020"), yend = 20),
                  arrow = arrow(length = unit(0.1, "cm"))) +
  geom_vline(xintercept=(dmy("1/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Local restrictions",y = 7, x = dmy("1/9/2020"), angle=90, size = 3, vjust= -0.5) +  
  geom_vline(xintercept=(dmy("13/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Universities re-open",y = 7, x = dmy("13/9/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_vline(xintercept=(dmy("14/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Rule of 6",y = 7, x = dmy("14/9/2020"), angle=90, size = 3, vjust= 1.5) +  
  geom_vline(xintercept=(dmy("25/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Pubs to close early",y = 7, x = dmy("25/9/2020"), angle=90, size = 3, vjust= -0.5) +   geom_vline(xintercept=(dmy("9/10/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Hospitality & \nCentral Belt restrictions",y = 8.5, x = dmy("9/10/2020"), angle=90, size = 3, vjust= -0.2, lineheight = 1) +geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Tier system",y = 7, x = dmy("2/11/2020"), angle=90, size = 3, vjust= -0.5) + 
  geom_errorbar(width = 0, colour = nice_green, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.1) +
  geom_line(colour = nice_green) +
  geom_point(colour = nice_green, size = 0.5) +
...
fig4 <- ggplot(wk_gr_hosp_icu %>% filter(!is.na(ratio_m) & 
  ...
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_vline(xintercept=(dmy("29/8/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Schools re-open (Aug 11)",y = 6.5, x = dmy("29/8/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_segment(aes(x = dmy("29/8/2020"), y = 40, xend = dmy("27/8/2020"), yend = 40),
                  arrow = arrow(length = unit(0.1, "cm"))) +
  geom_vline(xintercept=(dmy("1/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Local restrictions",y = 10, x = dmy("1/9/2020"), angle=90, size = 3, vjust= -0.5) +  
  geom_vline(xintercept=(dmy("13/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Universities re-open",y = 10, x = dmy("13/9/2020"), angle=90, size = 3, vjust= -0.5) +
  geom_vline(xintercept=(dmy("14/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Rule of 6",y = 10, x = dmy("14/9/2020"), angle=90, size = 3, vjust= 1.5) +  
  geom_vline(xintercept=(dmy("25/9/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Pubs to close early",y = 10, x = dmy("25/9/2020"), angle=90, size = 3, vjust= -0.5) +   geom_vline(xintercept=(dmy("9/10/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Hospitality & \nCentral Belt restrictions",y = 10, x = dmy("9/10/2020"), angle=90, size = 3, vjust= -0.2, lineheight = 1) +geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  geom_vline(xintercept=(dmy("2/11/2020")), colour="grey11", linetype = "dashed", size = 0.5) +
  annotate("text",label = "Tier system",y = 10, x = dmy("2/11/2020"), angle=90, size = 3, vjust= -0.5) + 
  geom_errorbar(width = 0, colour = nice_orange, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  geom_line(colour = nice_orange) +
  geom_point(colour = nice_orange, size = 0.5) +
...
png("weekly_ratios_hosp_icu_timeline.png", width = 9, height = 6, units = "in", res = 400)
gridExtra::grid.arrange(fig3, fig4, nrow = 2)
dev.off()
...
```{r deathsno, fig.width=8,fig.cap="Number of deaths per week, compared by location and data source", eval = FALSE}
ggplot(bind_rows(wk_gr_nps_nrs, wk_gr_newdeaths_latest) %>%
         mutate(Location = factor(Location, levels = c("PHS", "NRS","Care Home", "Hospital","Home / Non-institution"))) %>%
         mutate(Location2 = recode_factor(Location, "PHS" = "PHS", 
...
```{r fig.width=8.5,fig.height = 10.5, fig.cap="Daily cases by council area.",eval=FALSE}
ggplot(data_ca) +
...
```{r fig.width=8.5,fig.height = 10.5, fig.cap="Daily cases per 100,000 population by council area.", eval = FALSE}
data_ca %>% 
  filter(date >= dmy("1/3/2020")) %>%
  mutate(rolling_av = zoo::rollmean(new_cases_pop_100000, k = 7, align = "right", fill = NA)) %>%
  #mutate(rolling_av = case_when(health_board %in% c("Western Isles", "Orkney") ~ NA_integer_,
  #                               TRUE ~ as.integer(rolling_av))) %>%
ggplot() +
   geom_bar(aes(x=date, y = new_cases_pop_100000, fill = Health_board), stat = "identity", position="dodge") +
  #geom_line(aes(x = date, y = rolling_av), colour = "black", size = 0.2) +
  scale_fill_manual(values = c("#2f4f4f", "#006400","#8b0000","#000080", "#ff1493", "#ff8c00",#"#ffff00",
                               "gold1","#deb887", "#00ff00", "#00bfff", "#0000ff","#ff00ff", "#8b4513", "#dda0dd")) +
...
```{r fig.width=8.5,fig.height = 10.5, fig.cap="Daily deaths per 100,000 population by council area.", eval = FALSE}
ggplot(data_ca) +
...
```{r fig.width=8.5,fig.height = 10.5, fig.cap = "Weekly ratios for new cases for the last 21 days for each council area. Weekly ratios for new cases are ratios of weekly sums, plotted on a log10 scale. Dotted red lines highlight a weekly ratio value of 1.", eval = FALSE}
ggplot(wk_gr_newcases_ca %>% filter(date > max(data_ca$date)-20)) +
  ...
  facet_wrap(~CAName, strip.position = "top", scales = "free_y", ncol = 3) +
    theme(panel.spacing = unit(0, "lines"), 
         strip.background = element_blank(),
         strip.placement = "outside") +
  labs(fill = "", x = "", y = "New Cases Weekly Ratios") +
  theme(legend.position = "top",
        panel.background = element_blank(),
        axis.line = element_line(colour = "black"),
        axis.text = element_text(size = 6))
