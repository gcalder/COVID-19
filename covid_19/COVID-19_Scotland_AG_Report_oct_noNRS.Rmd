...
geo_label <- recode(ca_map$geo_label, "Edinburgh, City of" = "City of Edinburgh", 
...
```{r casesdeathsposrate, fig.width=11, fig.height=7, fig.cap = "Daily cases, deaths and positivity rate and 7 day rolling averages. Positivity rate refers to the percentage of people testing positive out of the total number of people tested each day."}
...
```{r, fig.width=7, fig.cap="Map showing weekly ratio by Local Authority", eval = FALSE}
wk_gr_newcases_ca_4map <- wk_gr_newcases_ca %>% filter(date > max(data_ca$date)-7 & date < max(data_ca$date)-3)
ca_map_simp2 <- ca_map_simp %>%
  left_join(select(wk_gr_newcases_ca_4map, CAName, date, ratio_m), by = c("geo_label" = "CAName"))
