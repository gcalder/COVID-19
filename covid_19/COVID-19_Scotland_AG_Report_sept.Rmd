...
```{r, fig.width=11, fig.height=7, dailydeathscases, fig.cap = "Daily cases, deaths and % positive and 7 day rolling average. Positivity rate refers to the percentage of people testing positive out of the total number of people tested each day."}
...
ggplot() + 
  ...
  scale_x_date(date_breaks = "1 week", labels = date_format("%b %d"), 
               limits = c(max(scot_data$date)-20, max(scot_data$date))) +
  theme_bw(base_size = 10) +
  labs( x = "", y = "") +
  theme_bw(base_size = 10) +
  theme(legend.position = "none",
...
```{r dailyoccupancy, fig.width=9, fig.height=5, fig.cap="Number of patients in hospital and ICU at midnight. Daily totals include both confirmed and suspected COVID-19 cases for ICU, but only confirmed cases for the total hospital in-patients", eval = FALSE}
fig6 <- data_hosp %>%
...
```{r deathscaseshospicu, fig.width=6, fig.height= 7.5, fig.cap="Weekly ratios for new cases and new deaths are ratios of weekly sums, plotted on a log10 scale. Entries in grey are adjusted for public holiday effects, see Caveats. Figure insets highlight ratios and trends for the last 21 days."}
...
fig1 <- ggdraw() +
  draw_plot(fig1) +
  draw_plot(fig1_inset, x = 0.45, y = .65, width = .44, height = .24)
...
fig2 <- ggdraw() +
  draw_plot(fig2) +
  draw_plot(fig2_inset, x = 0.45, y = .65, width = .44, height = .24)
...
```{r ratiosnrshpsloc, fig.width=8,fig.cap="Ratio of new deaths during current week vs previous week, compared by location and data source. Figure inset highlights the last date that both PHS and NRS data are available for."}
...
```{r ratiosnrshpsdaily, fig.width=8.5, fig.height=6, fig.cap="Ratio of new deaths during current week vs previous week, compared by data source. Entries in grey are adjusted for public holiday effects, see Caveats. Figure insets highlight ratios and trends for the last 21 days."}
# wk_gr_newdeaths_nrs_hps %>% 
#   filter(!is.na(ratio) & date >= dmy("26/04/2020")) %>%
# mutate(Ratio = formatC(round(ratio_m,2), format='f', digits=2),
#          lci = formatC(round(lci,2), format='f', digits=2),
