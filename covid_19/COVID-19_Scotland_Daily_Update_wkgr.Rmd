...
The number of new deaths reported today was `r scot_deaths$new_deaths[nrow(scot_deaths)]`, which represents a `r formatC(100*(scot_deaths$deaths[nrow(scot_deaths)]-scot_deaths$deaths[nrow(scot_deaths)-1])/scot_deaths$deaths[nrow(scot_deaths)-1], format='f', digits=2 )`% increase on yesterday's total. <br/> 
...
The number of new cases today is `r scot_data$new_cases[nrow(scot_data)]`, which represents a `r formatC(100*(scot_data$confirmed_cases[nrow(scot_data)]-scot_data$confirmed_cases[nrow(scot_data)-1])/scot_data$confirmed_cases[nrow(scot_data)-1], format='f', digits=2 )`% increase on yesterday's total. <br/> 
...
The day-of-the-week ratio for hospital occupancy `r formatC(wk_gr_hosp_hosp_latest$ratio_m, format='f', digits=2 )`. This is `r wk_gr_hosp_hosp_latest$significance`. <br/>
...
The highest incidence over the last day was `r formatC(max(scot_ti_hb$today_incidence), format='f', digits=2 )`, which occurred in `r subset(scot_ti_hb, today_incidence == max(scot_ti_hb$today_incidence))$health_board`. <br/>
...
The largest relative increase in cases was `r formatC(max(data_incidence_today$relative_increase_cases),format='f', digits=2)`%, which occurred in `r subset(data_incidence_today, relative_increase_cases == max(data_incidence_today$relative_increase_cases))$health_board`. <br/> 
...
fig2 <- ggplot(wk_gr_newdeaths %>% filter(!is.na(ratio) & date >= dmy("23-03-2020"))) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_red, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
...
fig2_inset <- ggplot(wk_gr_newdeaths %>% filter(!is.na(ratio) & date >= max(wk_gr_newdeaths$date)-20)) +
  aes(x = date, y = ratio,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, colour = nice_red, alpha = 0.4) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
