...
scot_deaths <- data_deaths %>%
  ...
  mutate(new_deaths = case_when(new_deaths == 207 & date == dmy("3/3/2021") ~ 35, 
                                TRUE ~new_deaths))
#Delete this
...
#Remove silly new 'super groups'
data_agr <- data_agr [(!(data_agr$AgeGroup=="0 to 59") & !(data_agr$AgeGroup=="60+")),]
...
#Re-arrange the age groups so we can have WRs for 75+

#Filter the data we actually need for WCR by age group
data_agr_total_2 <- data_agr_total %>%
  select(date,AgeGroup, new_cases)

#Rename the age groups over 75 so that they are the same
data_agr_total_2$AgeGroup[data_agr_total_2$AgeGroup == "75 to 84"] <- "75 Plus"
data_agr_total_2$AgeGroup[data_agr_total_2$AgeGroup == "85plus"] <- "75 Plus"

#Group newCases by AgeGroup and Date
data_agr_total_2b <- data_agr_total_2 %>%
	group_by(AgeGroup, date) %>%
	summarise(new_cases= sum(new_cases))

wk_gr_newcases_agr <- data_agr_total_2b %>% rename("outcome" = new_cases) %>%
  arrange(date) %>%
  group_by(AgeGroup) %>%
  nest() %>%
...
Death numbers reported by PHS include individuals who have positive tests but are not in hospital. Increased testing in the community (particularly in care homes) will cause more deaths to be counted and make PHS death numbers difficult to compare over time. The NRS death numbers are expected to provide a more consistent comparison, and also to allow care home and other non-hospital deaths to be considered separately. However, these data are less up-to-date than the data from PHS and are currently excluded from this report<!-- (Figures \@ref(fig:ratiosnrshpsloc) and \@ref(fig:ratiosnrshpsdaily))-->. <!--The latest weekly ratio for NRS deaths, `r filter(latest_nrs_hps, Location == "NRS")$ratio_m` (95% CI `r filter(latest_nrs_hps, Location == "NRS")$lci`-`r filter(latest_nrs_hps, Location == "NRS")$uci`), is lower than the equivalent value for PHS deaths `r filter(latest_nrs_hps, Location == "PHS")$ratio_m` (95% CI `r filter(latest_nrs_hps, Location == "PHS")$lci`-`r filter(latest_nrs_hps, Location == "PHS")$uci`) and might be expected to provide a more accurate reflection of the underlying state of the epidemic (based on NRS data up to `r latest_nrs_hps_date`; Figures \@ref(fig:ratiosnrshpsloc) and \@ref(fig:ratiosnrshpsdaily)).-->
...
fig1 <- ggdraw() +
  draw_plot(fig1) +
  draw_plot(fig1_inset, x = 0.16, y = .67, width = .41, height = .28) 
...
fig2 <- ggdraw() +
  draw_plot(fig2) +
  draw_plot(fig2_inset, x = 0.16, y = .67, width = .41, height = .28)
...
fig1_inset <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & 
  ...
  geom_line(colour = nice_blue) +
  geom_point(colour = nice_blue, size = 1) +
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d"),expand = expansion(mult = 0.1)) +
  scale_y_continuous(trans = "log10") +
  labs( x = "", y = "") +
...
fig1_inset <- ggplot(wk_gr_newcases %>% filter(!is.na(ratio) & 
  ...
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
  scale_y_continuous(trans = "log10", #limits = c(0.5, 1.30),
                     breaks = c(0.5,0.75,1, 1.25, 1.75,2.25),
                     labels = c(0.5,0.75,1, 1.25, 1.75,2.25)) +
  labs( x = "", y = "") +
  theme_bw(base_size = 8) +
  theme(plot.margin = margin(0, 0, -1, 0, "cm")) 
fig1 <- ggdraw() +
...
fig3_inset <- ggplot(wk_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
  ...
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
  scale_y_continuous(trans = "log10", #limits = c(0.45, NA),
                     breaks = c(0.5,0.75,1,1.25, 1.75),
                     labels = c(0.5,0.75,1,1.25, 1.75)) +
  labs( x = "",
        y = "") +
  theme_bw(base_size = 8) +
  theme(plot.margin = margin(0, 0, -1, 0, "cm"))
...
fig4_inset <- ggplot(wk_gr_hosp_icu %>% filter(!is.na(ratio_m) & 
  ...
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
  scale_y_continuous(trans = "log10", #limits = c(0.18, NA),
                     breaks = c(0.1,0.5,1,3,5),
                     labels = c(0.1,0.5,1,3,5)) +
  labs( x = "",
        y = "") +
  theme_bw(base_size = 8) +
  theme(plot.margin = margin(0, 0, -1, 0, "cm"))
...
figX <- ggplot(wk_gr_newcases_agr_plt) +
  aes(x = date, y = ratio,
  ...
  scale_y_continuous(trans=scales::pseudo_log_trans(base = 10), breaks = c(0,0.5,1,2,4,8), labels = c(0,0.5,1,2,4,8),
                     #limits = c(0.5,8),
expand = expansion(mult = 0.1)) +
  #scale_color_brewer(palette = "Dark2") +
  scale_color_manual(values = c("#90ee90","#4DAF4A","#377eb8","#984ea3","#f781bf","#FF7F00","#e41a1c","#000000")) +
  labs( x = "End Date of Current Week",
        y = "Weekly Ratio",
        #title = "Change in weekly cases/deaths total as ratio of previous week",
        subtitle = "New Cases (Source: PHS)") +
...
figX_inset <- ggplot(wk_gr_newcases_agr_plt%>% filter(!is.na(ratio_m) & !is.infinite(ratio) & date >= max(wk_gr_newcases_agr_plt$date)-20)) +
  aes(x = date, y = ratio,ymin = lci, ymax = uci, colour = AgeGroup) +
  geom_hline(yintercept = 1, linetype = "dashed", size = 0.2) +
  geom_line() +
  geom_point( size = 1) +
  scale_x_date(date_breaks = "1 week", labels = date_format("%b %d")) +
  scale_y_continuous(trans=scales::pseudo_log_trans(base = 10),limits = c(NA,NA)
 ,breaks = c(0.5,1,2,3),
  expand = expansion(mult = 0.1)) +
  scale_color_manual(values = c("#90ee90","#4DAF4A","#377eb8","#984ea3","#f781bf","#FF7F00","#e41a1c","#000000")) +
  labs(x = "", y = "") +
...
figX <- ggdraw() +
  draw_plot(figX) +
  draw_plot(figX_inset, x = 0.40, y = .66, width = .44, height = .25)
figX
```
...
#write.csv(wk_gr_newcases_adj,"/Users/gcalder2/OneDrive - University of Edinburgh/COVID-19_Scot_NewCases.csv")
#write.csv(wk_gr_newcases_adj,"COVID-19_Scot_NewCases.csv")

#write.csv(wk_gr_hosp_hosp, "COVID-19_Scot_HospCheck.csv")

#str(wk_gr_newcases_agr_plt)

#wk_gr_newcases_agr_plt2 <- select(wk_gr_newcases_agr_plt, -data)

#sapply(wk_gr_newcases_agr_plt2, typeof)


#write.csv(wk_gr_newcases_agr_plt2, "COVID-19_Scot_WKagr.csv")

#write.csv(wk_gr_hosp_hosp, "COVID-19_Scot_WR_hosp.csv")



#write.csv(data_hosp, "COVID-19_Scot_WR_hosp_raw.csv")
~
