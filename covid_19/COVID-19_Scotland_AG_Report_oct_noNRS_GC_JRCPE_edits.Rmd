...
date: "`r format(Sys.time()-lubridate::days(0), '%d %B, %Y')`" 
header-includes:
   - \usepackage{booktabs}
   - \usepackage{float}
...
fig1 <- ggdraw() +
  draw_plot(fig1) +
  draw_plot(fig1_inset, x = 0.37, y = .64, width = .41, height = .31) 
...
wk_gr_newcases <- wk_gr_newcases %>% filter(date <= dmy("21-01-2021")) 
...
fig1 <- ggdraw() +
  draw_plot(fig1) +
  draw_plot(fig1_inset, x = 0.523, y = .66, width = .44, height = .20)

wk_gr_newdeaths_adj_mainplot <- wk_gr_newdeaths_adj_mainplot %>% filter(date <= dmy("21-01-2021")) 
...
fig2_inset <- ggplot(wk_gr_newdeaths_adj_mainplot %>% filter(!is.na(ratio) & 
              !is.infinite(ratio) & date >= max(wk_gr_newdeaths_adj_mainplot$date)-20)) +
  aes(x = date, y = ratio_m,
    ymin = lci, ymax = uci) +
  geom_errorbar(width = 0, alpha = 0.4, aes(colour = ci)) +
  geom_hline(yintercept = 1, colour = "red", linetype = "dashed", size = 0.2) +
  ...
  scale_fill_manual(values = c("grey50", nice_red)) +
    scale_y_continuous(trans = "log10", breaks = c(0.7,1,1.5,2.2)) +
  labs( x = "",
        y = "") +
  theme_bw(base_size = 8) +
  theme(plot.margin = margin(0, 0, -1, 0, "cm"), 
...
fig2 <- ggdraw() +
  draw_plot(fig2) +
  draw_plot(fig2_inset, x = 0.523, y = .66, width = .44, height = .20)

wk_gr_hosp_hosp <- wk_gr_hosp_hosp %>% filter(date <= dmy("21-01-2021")) 
...
fig3_inset <- ggplot(wk_gr_hosp_hosp %>% filter(!is.na(ratio_m) & 
  ...
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
  scale_y_continuous(trans = "log10", limits = c(0.95, NA),
                     breaks = c(1,1.3,1.6)) +
  labs( x = "",
        y = "") +
  theme_bw(base_size = 8) +
...
fig3 <- ggdraw() +
  draw_plot(fig3) +
  draw_plot(fig3_inset, x = 0.523, y = .66, width = .44, height = .20)

wk_gr_hosp_icu <- wk_gr_hosp_icu %>% filter(date <= dmy("21-01-2021")) 
...
fig4 <- ggdraw() +
  draw_plot(fig4) +
  draw_plot(fig4_inset, x = 0.523, y = .66, width = .44, height = .20)
...
png(filename = "ca_rations_compy.png", width = 16, height = 10, units = "in", res = 300)
a <- ggplot(wk_gr_newcases_ca %>% filter(date > max(data_ca$date)-25 & date < max(data_ca$date)-3) %>%
         filter(!is.na(ratio) & !is.infinite(ratio))) +
  ...
  scale_colour_manual(values = c("#2f4f4f", "#006400","#8b0000","#000080", "#ff1493", "#ff8c00",#"#ffff00",
                      ...
                      guide = guide_legend(override.aes = list(linetype = 0,
                                                                  shape = 15, 
                                                               size = 6))) + 
  #scale_y_continuous(trans = "log10", expand = expansion(mult = c(0, 0.05))) +
  ...
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
   ...
   #                      #limits = c(NA, 20),
   #                      expand = expansion(mult = 0.1)) +
    # scale_y_continuous(trans = "log10", breaks = trans_breaks("log10", function(x) 10^x), labels = trans_format("log10",function(x) 10^x)) +
...
b <- ggplot(wk_gr_newcases_ca %>% filter(date > max(data_ca$date)-25 & date < max(data_ca$date)-3) %>%
         filter(!is.na(ratio) & !is.infinite(ratio))) +
  #geom_rect(data=data.frame(xmin = max(data_ca$date)-3, xmax = max(data_ca$date), ymin = -Inf, ymax = Inf),
    #aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)) +
  geom_point(aes(x=date, y = ratio_m, colour = Health_board), size = 1) +
  ...
  #coord_trans(y = "log10", expand = expansion(mult = c(0, 0.05))) +
  #scale_y_continuous(expand = expansion(mult = c(0, 0.05))) +
  scale_x_date(date_breaks = "7 days", labels = date_format("%b %d")) +
   # scale_y_continuous(trans = "log10",
   #                      #breaks = c(0.2, 0.5, 1, 3, 10, 30),
...
c <- ggplot(wk_gr_newcases_ca %>% filter(date > max(data_ca$date)-25 & date < max(data_ca$date)-3) %>%
         filter(!is.na(ratio) & !is.infinite(ratio))) +
  #geom_rect(data=data.frame(xmin = max(data_ca$date)-3, xmax = max(data_ca$date), ymin = -Inf, ymax = Inf),
    #aes(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax)) +
  geom_point(aes(x=date, y = ratio_m, colour = Health_board), size = 1) +
