...
```{r dailyoccupancy, fig.width=6, fig.height=3, fig.cap="Number of patients in hospital and ICU at midnight. Daily totals include both confirmed and suspected COVID-19 cases"}
fig1 <- data_hosp_icu %>%
  group_by(date) %>%
...
fig2_inset <- ggplot(wk_gr_newdeaths %>% filter(!is.na(ratio) & 
  ...
  geom_line(colour = nice_red) +
  geom_point(colour = nice_red, size = 1) +
  scale_x_date(date_breaks = "3 days", labels = date_format("%b %d"),expand = expansion(mult = 0.1)) +
  scale_y_continuous(trans = "log10") +
  labs( x = "",
        y = "") +
  theme_bw(base_size = 8) +
