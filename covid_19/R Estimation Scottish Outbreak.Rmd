...
R estimation over the preceding 7 days using R package [EpiEstim](https://cran.r-project.org/web/packages/EpiEstim/vignettes/demo.html), based on daily cases reported.Their methos is described in [this publication](https://www.sciencedirect.com/science/article/pii/S1755436519300350).
...
fig1_inset <- ggplot(filter(df_all, date > max(df_all$date)-8), aes(x = date, y = `Mean(R)`, ymin = `Quantile.0.025(R)`, ymax = `Quantile.0.975(R)`, 
  ...
  geom_line(aes(colour = factor(si))) + 
  geom_hline(yintercept = 1, linetype = "dashed") +
  scale_y_continuous(breaks = c(0.7,0.75,0.8, 0.85, 0.9, 0.95, 1.0, 1.05, 1.1)) +
