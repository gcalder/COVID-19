...
gr_newcases <- growth_rates(dat = scot_data %>% rename("outcome" = new_cases)) %>%
               ...
               significance()
gr_newdeaths <- growth_rates(dat = scot_deaths %>% rename("outcome" = deaths)) %>%
               filter(date >= ymd("2020-03-23")) %>%
               significance()
gr_newcases_hb <- scot_data_health_board %>% rename("outcome" = confirmed_cases) %>%
   group_by(health_board) %>%
   nest() %>%
...
The average daily growth rate for new deaths measured over the last 7 days was `r paste0(gr_newdeaths$sign[nrow(gr_newdeaths)], formatC(round(100*(gr_newdeaths$growth[nrow(gr_newdeaths)]-1),1), format='f', digits=1 ))`%, which is `r gr_newdeaths$significance[nrow(gr_newdeaths)]`.  
...
The average daily growth rate for new cases measured over the last 7 days was `r paste0(gr_newcases$sign[nrow(gr_newcases)], formatC(round(100*(gr_newcases$growth[nrow(gr_newcases)]-1),1), format='f', digits=1 ))`%, which is `r gr_newcases$significance[nrow(gr_newcases)]`. 
...
The fastest average daily growth rate for new cases over the past 7 days was `r paste0(data_growth_rates_today$sign, formatC(round(100*(data_growth_rates_today$growth-1),1), format='f', digits=1))`%, which is `r data_growth_rates_today$significance` and occurred in `r data_growth_rates_today$health_board`.
...
fig1 <- ggplot(gr_newcases) +
  ...
  scale_y_continuous(expand = c(0.1, 0), breaks = c(-0.2, -0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8), labels = scales::percent_format(), limits = c(-0.2, NA)) +
  labs(
...
fig2 <- ggplot(gr_newdeaths) +
  ...
  scale_y_continuous(expand = c(0.1, 0), breaks = c(-0.1, 0, 0.1, 0.2, 0.3, 0.4, 0.6, 0.8), labels = scales::percent_format(), limits = c(-0.011, NA)) +
  labs(
    x = "End date of 7 day period",
    y = "Daily % growth",
    subtitle = "New deaths"
...
ggplot(gr_newcases_hb) +
  ...
  geom_line() +
  geom_point() +
  scale_y_continuous(expand = c(0.1, 0), breaks = c(-0.10, 0, 0.2, 0.4, 0.6, 0.8), labels = scales::percent_format(), limits = c(-0.11, NA)) +
  scale_color_manual(values = c('#e6194b', '#ffe119', '#4363d8','#800000', '#f58231', '#911eb4', '#46f0f0', '#f032e6', '#bcf60c', '#008080', '#9a6324',  '#808000', '#ffd8b1', '#000075', '#808080'))+
