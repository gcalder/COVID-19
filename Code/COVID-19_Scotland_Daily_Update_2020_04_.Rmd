...
# Import Scottish Population Data
...
# Scottish cases data
scot_data_raw <- read_excel("COVID-19_Scotland_data_all_2020-04-08.xlsx", sheet = "Cases By Health Board", skip = 1)
...
scot_ci_hb <- read_excel("COVID-19_Scotland_data_all_2020-04-08.xlsx", sheet = "Cumulative Incidence Grouped") %>%
  slice(nrow(.)) %>%
...
scot_ti_hb <- read_excel("COVID-19_Scotland_data_all_2020-04-08.xlsx", sheet = "Incidence by Health Board") %>%
  slice(nrow(.)) %>%
  select(Ayrshire:Tayside) %>%
...
# Scottish death data
scot_deaths <- read_excel("COVID-19_Scotland_data_all_2020-04-08.xlsx", sheet = "Scotland Deaths", skip = 1) %>%
...
# Scottish tests
scot_tests <- read_excel("COVID-19_Scotland_data_all_2020-04-08.xlsx", sheet = "CPT & DPC") 
...
The number of new deaths reported today was `r scot_deaths$new_deaths[nrow(scot_deaths)]`, which represents a `r round(100*(scot_deaths$deaths[nrow(scot_deaths)]-scot_deaths$deaths[nrow(scot_deaths)-1])/scot_deaths$deaths[nrow(scot_deaths)-1],1)`% increase on yesterday's total. <br/>
The doubling time for deaths over the past 7 days was `r round(scot_deaths$doubling_time_week[nrow(scot_deaths)],1)` days.

**Cases:** <br/>
The total number of cases is currently `r comma(scot_data$confirmed_cases[nrow(scot_data)])`. <br/> 
The number of new cases today is `r scot_data$new_cases[nrow(scot_data)]`, which represents a `r round(100*(scot_data$confirmed_cases[nrow(scot_data)]-scot_data$confirmed_cases[nrow(scot_data)-1])/scot_data$confirmed_cases[nrow(scot_data)-1],1)`% increase on yesterday's total. <br/>
The doubling time for the number of cases over the past 7 days was `r round(scot_data$doubling_time_week[nrow(scot_data)],1)` days.

```{r}
data_incidence <- scot_data_health_board %>%
  left_join(scot_pop, by = c("health_board" = "Name")) %>%
  mutate(cumulative_incidence = 10000* confirmed_cases/Population, 
         day_incidence = 10000*new_cases/Population) %>%
  group_by(health_board) %>%
  arrange(date) %>%
...
The highest incidence over the last day was `r round(max(scot_ti_hb$today_incidence),1)`, which occurred in `r subset(scot_ti_hb, today_incidence == max(scot_ti_hb$today_incidence))$health_board`. <br/>
The highest cumulative incidence is `r round(max(scot_ci_hb$cumulative_incidence),1)`, which is in `r subset(scot_ci_hb, cumulative_incidence == max(scot_ci_hb$cumulative_incidence))$health_board`.
...
The largest relative increase in cases was `r round(max(data_incidence_today$relative_increase_cases),1)`%, which occurred in `r subset(data_incidence_today, relative_increase_cases == max(data_incidence_today$relative_increase_cases))$health_board`. <br/>
The fastest doubling time over the past 7 days was `r format(round(min(subset(data_incidence_today, doubling_time_week>0)$doubling_time_week)),nsmall = 1)` days, which occurred in `r subset(data_incidence_today, doubling_time_week == min(subset(data_incidence_today, doubling_time_week>0)$doubling_time_week))$health_board`.
...
The percentage of tests which were positive is currently `r round(100*scot_tests[nrow(scot_tests),"Total Positive"]/scot_tests$Conducted[nrow(scot_tests)],1)[[1]]`%.
...
```{r}
plot_ly(data = scot_tests, x = ~Date, y = ~cases_per_test, color = ~I(nice_blue), type = "scatter", mode = "lines+markers", 
               hovertemplate = paste('Date: %{x}',
...
```{r}
plot_ly(data = scot_tests, x = ~Date, y = ~deaths_per_case, color = ~I(nice_red), type = "scatter", mode = "lines+markers", 
               hovertemplate = paste('Date: %{x}',
